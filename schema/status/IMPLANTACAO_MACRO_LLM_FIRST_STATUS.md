...
**2026-05-15** — Fix complementar T11.1 aplicado: reset defensivo de `_diag` no início de `extractFactsFromText` previne vazamento PII cross-lead via `getLastExtractionDiagnostics()` stale. Smoke regressivo S-DIAG-REGRESSION (5 checks). Identificado por Codex na PR #1 do staging, replicado para prod via cherry-pick.

**T11.1 — telemetria estrutural completa: _diag branch/skipped/keywords, classifyNextObjectiveFormat, invariant_check** (2026-05-09) — PR aberta (branch feat/t11.1-telemetria-estrutural, commit 3c5c2a7, aguardando merge Vasques); PR-DIAG telemetria estrutural; frente T11; contrato: `schema/contracts/active/CONTRATO_MESTRE_T11.md` §5 T11.1; **mudanças**: (1) `text-extractor.ts` — módulo `_diag` + `getLastExtractionDiagnostics()` + reset por chamada + 30 marcadores de branch (first-set-wins) + 3 keyword pushes + 1 skipped push; (2) `canary-pipeline.ts` — `classifyNextObjectiveFormat()` + `extractionDiag` capture + 5 novos campos em `text_extractor.result` + 4 novos campos em `core.decision` + novo log `pipeline.invariant_check`; zero mudança comportamental; **testes**: smoke:core:text-extractor **124/124 PASS**, smoke:meta:canary **41/41 PASS**; **próxima ação T11**: Vasques merge T11.1 → E2E pós-merge → preencher Bloco E → T11.2 (unificar AVANCAR_PARA_CIVIL para código curto).

**T9.29 — diagnóstico runtime: pending_objective em log text_extractor.result** (2026-05-09) — PR aberta (branch fix/t9.29-diag-pending-objective, aguardando merge Vasques); PR-DIAG telemetria; frente T9; **mudança**: 2 campos adicionados ao diagLog `text_extractor.result` em `canary-pipeline.ts:268` — `pending_objective` (string|null) e `pending_objective_length` (number) — sem nenhuma alteração comportamental; **testes**: smoke:core:text-extractor **124/124 PASS**, smoke:meta:canary **41/41 PASS**; **próxima ação T9: Vasques merge PRs T9.26+T9.27+T9.28+T9.29 → Repetir E2E C1-C8**.

**T9.28 — 3 gaps C4/C6: guard P3 estado_civil, nacionalidade conservadora, normalizeAlternativaRnm expande** (2026-05-09) — PR #265 aberta (branch fix/t9.28-gaps-c4-c6, aguardando merge Vasques); PR-IMPL correcao_incidental; frente T9; diagnóstico base: `docs/diagnostics/FUNIL-QUALIFICACAO/32-gap-analysis-c4-c6.md`; **FIX 1** (GAP-C4): guard `pendingObjective !== 'coletar_estado_civil_p3'` adicionado ao bloco keyword `estado_civil` em `extractQualificationCivil` — elimina contaminação de `estado_civil` do lead quando cliente responde ao `estado_civil_p3` da composição; **FIX 2** (GAP-C6A): bloco `nacionalidade` em `extractDiscovery` substituído por contextual (`perguntar_nacionalidade`) + fallback conservador (`sou brasileiro`/`sou estrangeiro` em vez de `brasileiro`/`estrangeiro` bare) — "Nao tenho familiar brasileiro" não captura mais `nacionalidade=brasileiro`; **FIX 3** (GAP-C6B): `normalizeAlternativaRnm` em `topo-parser.ts` expandida para reconhecer `sem_familiar_brasileiro` e `sem_conjuge_brasileiro` (ambos mapeados para `sem_alternativa`) — Gate 4B agora dispara corretamente e encerra fluxo RNM; **4 smoke tests CTX37-CTX40** (CTX37a/b, CTX38a/b, CTX39, CTX40); **testes**: smoke:core:text-extractor **124/124 PASS** (era 118); **próxima ação T9: Vasques merge PRs T9.26+T9.27+T9.28 → Repetir E2E C1-C8**.

**T9.27 — 4 fixes residuais E2E: renda flexivel, estado_civil_p3 discovery, rnm 3-tier, alternativa_rnm granular** (2026-05-09) — PR aberta (branch fix/t9.27-residuais-e2e, aguardando merge Vasques); PR-IMPL correcao_incidental; frente T9; 4 bugs confirmados via E2E e discovery doc; **FIX 1** (`parseRendaFlexivel`): nova função substitui `extractRenda` nos dois call-sites de `renda_principal` (em `extractQualificationRenda` e no bloco cross-stage de `extractQualificationCivil`) — aceita número puro ("2500"), extensos ("três mil", "três mil e quinhentos"), notação k ("3k") além dos formatos existentes; **FIX 2** (`estado_civil_p3` em discovery): bloco adicionado em `extractDiscovery` com guard `=== undefined` + 2 pendingObjectives (opaco e semântico) — captura quando stage ainda é discovery e LLM já perguntou estado civil do familiar; guard implícito preservado: bloco `estado_civil` não dispara para `coletar_estado_civil_p3`; **FIX 3** (rnm_valido 3-tier): bloco contextual em `extractDiscovery` reescrito com 4 camadas (específico negativo → explícito negativo → específico positivo → genérico) — elimina risco de "Sim tenho mas tem validade" retornar true; **FIX 4** (`alternativa_rnm` granular): bloco reescrito com guard `=== undefined` + semantic pendingObjective + 5 valores: `sem_familiar_brasileiro` (antes: sem cobertura), `sem_conjuge_brasileiro` (novo), `sem_alternativa` (genérico preservado), `tem_conjuge_brasileiro` (novo), `tem_familiar_brasileiro` (antes: tudo era esse); **6 novos smoke tests CTX31-CTX36**; **testes**: smoke:core:text-extractor **118/118 PASS** (era 111); **próxima ação T9: Vasques merge PR T9.27 → Repetir E2E**.

**T9.26 — Captura cross-stage regime_trabalho/renda_principal + rnm negativas-primeiro + estado_civil_p3 semantic** (2026-05-08) — PR #263 (branch fix/t9.26-captura-cross-stage, aberta — aguardando merge Vasques); PR-IMPL correcao_incidental; frente T9; 4 fixes em `text-extractor.ts`: FIX1 (`estado_civil_p3` — add semantic pendingObjective + casado_civil em ordem correta + uniao_estavel sem junto/junta/namorado/namorada); FIX2 (regime_trabalho cross-stage em `extractQualificationCivil` para pendingObjectives `avancar_para_qualification_renda`/`coletar_regime_trabalho` + semânticos); FIX3 (renda_principal cross-stage idem, usa `extractRenda(original)` — `extractQualificationCivil` agora recebe `original: string`); FIX4 (`rnm_valido` contextual — negativas primeiro: `nao tenho` antes de `tenho` + guard `=== undefined` + semantic alternative); **6 novos smoke tests CTX25-CTX30**; **testes**: smoke:core:text-extractor **111/111 PASS** (era 105), smoke PASS, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34; zero diff fora do escopo; **próxima ação T9: Vasques merge PR #263 → Repetir T9.15B-PROVA-REAL-CANARY**.

**T9.24 — Reativar captura estado_civil em discovery com pendingObjective novo (T9.21)** (2026-05-09) — PR #262 (branch fix/t9.24-estado-civil-discovery-novo, aberta — aguardando merge Vasques); PR-IMPL correcao_incidental; frente T9; causa raiz: T9.23 removeu bloco de estado_civil em discovery porque pendingObjectives estavam desatualizados; T9.21 mudou semantic mapper de `avancar_para_qualification_civil` para perguntar estado civil (não processo); como stage muda apenas no próximo turno, extractor precisa capturar `estado_civil` em discovery quando LLM já fez a pergunta; **fix**: bloco `estado_civil` contextual adicionado em `extractDiscovery` (após bloco processo) com guard `=== undefined` + 3 pendingObjectives (semântico T9.21, opaco `avancar_para_qualification_civil`, direto `coletar_estado_civil`) + 6 ramos com especificidade `casado no civil` antes de `casado`; **arquivos modificados (2)**: `text-extractor.ts`, `text-extractor-smoke.ts` (CTX24 adicionado); **testes**: smoke:core:text-extractor **105/105 PASS** (era 104), smoke PASS, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34; zero diff fora do escopo; **próxima ação T9: Vasques merge PR #262 → Repetir T9.15B-PROVA-REAL-CANARY com estado_civil capturado em discovery**.

**T9.16B — RNM Alternativa + Greeting para topo vazio** (2026-05-05) — PR #241 (branch fix/t9.16b-rnm-alternativa-greeting, aberta — aguardando merge Vasques); PR-IMPL contratual; frente T9; causa raiz: estrangeiro com `rnm_valido=false` ficava preso em loop — Gate 4 original (`!signals.rnm_valido`) disparava para `null` E `false`, mas `rnm_valido=false` não tinha rota de saída; lead novo com topo vazio recebia "Tem interesse em comprar?" em vez de greeting estruturado; **fix**: (1) Gate 4 original restrito a `=== null`; (2) Gates 4A/4B adicionados: `rnm_valido=false` sem alternativa → `VERIFICAR_ALTERNATIVA_RNM`; `rnm_valido=false` + `alternativa_rnm='sem_alternativa'` → `ENCERRAR_SEM_ALTERNATIVA_RNM`; `rnm_valido=false` + alternativa confirmada (`tem_conjuge_brasileiro`/`tem_familiar_brasileiro`) → avança para `qualification_civil`; (3) Gate 1 atualizado: `parse_status='empty'` → `APRESENTAR_E_VERIFICAR_CONHECIMENTO` (greeting estruturado); (4) `TopoSignals` recebe campo `alternativa_rnm: AlternativaRnm | null`; (5) `text-extractor.ts` extrai `alternativa_rnm` contextualmente (via `pendingObjective='verificar_alternativa_rnm'`) + keywords diretas; (6) 3 novos mapeamentos em `semantic-next-objective.ts`; (7) `isTopoFactoCriticoAusente` atualizado para bloquear enquanto alternativa não for confirmada; **arquivos modificados (6)**: `topo-rules.ts`, `topo-parser.ts`, `topo-gates.ts`, `text-extractor.ts`, `semantic-next-objective.ts`, `smoke.ts`; **testes**: smoke **28/28 PASS** (era 24), smoke:core:text-extractor 89/89, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34, prove:t9.14-reverse-mapper 19/19; zero diff fora do escopo; **próxima ação T9: Vasques merge PR #241 → Repetir T9.15B-PROVA-REAL-CANARY com topo completo**.

**T9.16A — Confirmações contextuais no text-extractor — pendingObjective via _next_objective** (2026-05-05) — PR #240 (branch fix/t9.16a-contextual-confirmation, aberta — aguardando merge Vasques); PR-IMPL contratual; frente T9; causa raiz: `extractFactsFromText` recebia apenas `text` e `stage` — sem contexto da pergunta anterior; quando LLM perguntava sobre RNM e cliente respondia "sim", extractor não associava ao fact correto; **fix**: (1) `extractFactsFromText`, `extractDiscovery`, `extractQualificationCivil` recebem `pendingObjective?: string`; (2) blocos contextuais adicionados em `extractDiscovery` (RNM — 8 keywords positivos, 6 negativos) e `extractQualificationCivil` (estado_civil — 4 ramos, processo — 2 ramos); guards `facts['X'] === undefined` impedem sobrescrita do contextual pelo keyword-only; (3) `canary-pipeline.ts` deriva `pendingObjective` de `persistedFacts['_next_objective']`, passa para extractor, persiste `coreDecision.next_objective` como `_next_objective` em `factsMap`; (4) `semantic-next-objective.ts` — instrução RNM atualizada com regra prazo determinado/indeterminado; (5) 8 novos casos CTX1–CTX8 no smoke; **testes**: smoke:core:text-extractor **89/89 PASS** (era 81), smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34, prove:t9.14-reverse-mapper 19/19; zero diff fora do escopo; **próxima ação T9: Vasques merge PR #240 → Repetir T9.15B-PROVA-REAL-CANARY**.

**T9.15J-FIX2 — writeLeadAccumulatedFacts SELECT→upsert por id (PK)** (2026-05-05) — PR #239 (branch fix/t9.15j-fix2-upsert-by-id, aberta — aguardando merge Vasques); PR-FIX contratual; frente T9; causa raiz confirmada via file 29 (supabaseUpsert usa `Prefer: resolution=merge-duplicates` sem `on_conflict` explícito — PostgREST resolve conflict target por UNIQUE constraints do schema; `enova_state.wa_id` não tem UNIQUE constraint — file 28 confirmado — logo upsert por `{ wa_id, last_context, updated_at }` sempre INSERT nova linha, criando duplicata a cada turno); **fix**: `writeLeadAccumulatedFacts` reescrita — (1) SELECT por wa_id → obtém `id` (PK UUID) da row existente; (2) upsert por `id` (conflict target garantido por PK); degradação graceful: `ok=false, error='no_existing_row_for_wa_id'` se row ausente, `ok=false, error='existing_row_has_no_id'` se row sem PK; **testes**: smoke PASS, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34, smoke:core:text-extractor 81/81, prove:t9.14-reverse-mapper 19/19; zero diff fora do escopo; **próxima ação T9: Vasques merge PR #239 → Repetir T9.15B-PROVA-REAL-CANARY**.

**T9.15J — facts persistence: lead_id → wa_id em enova_state read/write** (2026-05-05) — PR #238 (branch fix/t9.15j-facts-persistence-wa-id, mergeada); PR-FIX contratual; frente T9; causa raiz confirmada via files 26-27: `mapLeadFromMeta` (crm-store.ts) seta `lead_id = wa_id` (número telefone texto ex: `"554185260518"`) pois `crm_lead_meta` tem `wa_id` como PK (TEXT UNIQUE) sem coluna UUID `lead_id`; `enova_state.lead_id` é UUID → Postgres rejeita com `pg_code=22P02 invalid input syntax for type uuid`; evidência real: wrangler tail (T9.15I) confirmou 100% das escritas falhando com pg_code=22P02 em PROD desde T9.15H; **fix**: (1) `readLeadAccumulatedFacts(cfg, wa_id: string)` — parâmetro e filtro alterados de `lead_id` para `wa_id`; (2) `writeLeadAccumulatedFacts(cfg, wa_id: string, facts)` — idem, upsert usa `wa_id` em vez de `lead_id`; (3) `canary-pipeline.ts` — 3 pontos: `readLeadAccumulatedFacts(supabaseCfg, event.wa_id ?? '')`, diagLog `wa_id_present: !!(event.wa_id)`, `writeLeadAccumulatedFacts(supabaseCfg, event.wa_id ?? '', factsMap)`; **testes**: smoke PASS, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34, smoke:core:text-extractor 81/81, prove:t9.14-reverse-mapper 19/19; zero diff fora do escopo; T9.15H nunca funcionou em PROD — T9.15J foi o primeiro write real bem-sucedido (após também merge T9.15J-FIX2 por PR #239).

**T9.15I — telemetria facts_persistence: catches observáveis + diagLogs read/write/core** (2026-05-05) — PR #237 (branch fix/t9.15i-facts-persistence-telemetry, mergeada); PR-DIAG+FIX contratual; frente T9; 2 arquivos funcionais modificados + 9 arquivos diagnósticos criados em `docs/diagnostics/T9.15I-DIAG-FACTS-PERSISTENCE-RUNTIME/`; **fix funcional** (`src/supabase/crm-store.ts` + `src/meta/canary-pipeline.ts`): catch em `readLeadAccumulatedFacts` agora emite `diagLog('facts_persistence.read')` antes de retornar `{}`; catch em `writeLeadAccumulatedFacts` capturado e retornado via `return { ok: false, error: String(e) }`; `diagLog('facts_persistence.read')` — `ok, persisted_count, persisted_keys, lead_id_present`; `diagLog('facts_persistence.write')` — `ok, facts_count, fact_keys, error`; `diagLog('core.facts_received')` — `facts_count, fact_keys, stage`; **diagnóstico** (files 23-29): files 23+25 — wrangler tail 2 turnos confirmou `facts_persistence.write: ok=false, error="http_400: pg_code=22P02"` em 100% das escritas PROD; files 26-27 — causa raiz rastreada: `mapLeadFromMeta` → `lead_id = wa_id`; file 28 — `enova_state.wa_id` sem UNIQUE constraint; file 29 — `supabaseUpsert` sem `on_conflict` explícito; **testes**: smoke PASS, smoke:meta:canary 41/41, prove:t9.15h-facts-persistence 34/34, smoke:core:text-extractor 81/81, prove:t9.14-reverse-mapper 19/19; próxima ação T9: T9.15J-FIX (lead_id → wa_id).

**T9.15H-FIX-FACTS-PERSISTENCE-TOPO IMPLEMENTADA** (2026-05-04) — PR-FIX contratual; frente T9; bug crítico de perda de facts entre turnos corrigido sem migration Supabase; causa raiz: `crm_facts` é writeBuffer-only em `SupabaseCrmBackend` — ao reiniciar isolate Cloudflare Worker, writeBuffer zera e facts do turno 1 (`customer_goal`) desaparecem no turno 2 → gate 1 falha → Enova pergunta interesse novamente em vez de pedir nacionalidade; **fix**: `enova_state.last_context` (coluna TEXT confirmada em T9.13G P0 sem migration) usada como storage JSON para facts acumulados entre turnos; **arquivos criados**: (1) `src/supabase/facts-persistence-proof.ts` — prova lógica 34/34 PASS (cenários A-G: merge básico, não-sobrescrita writeBuffer restart, acumulação 3 turnos, restart completo 4 facts, parse TEXT/JSONB/null/inválido/array, precedência turno atual, topo completo → autoriza qualification_civil); **arquivos modificados**: (2) `src/supabase/crm-store.ts` — `readLeadAccumulatedFacts(cfg, lead_id)` lê `enova_state.last_context` via `supabaseSelect`, parseia TEXT→JSON ou usa JSONB direto, retorna `{}` em falha silenciosa; `writeLeadAccumulatedFacts(cfg, lead_id, facts)` upserta `enova_state` com `last_context=JSON.stringify(facts)`; (3) `src/meta/canary-pipeline.ts` — Bloco [B]+[C] expandido: `readLeadAccumulatedFacts` antes do Core; merge `{ ...persistedFacts, ...extractedFacts }` (turno atual tem precedência); inject persistedFacts no writeBuffer apenas para keys não presentes no turno atual; `writeLeadAccumulatedFacts` após `upsertLeadState`; diagLog `text_extractor.result` + `facts_persistence.write` atualizados; (4) `package.json` — `prove:t9.15h-facts-persistence` adicionado; **testes**: `prove:t9.15h-facts-persistence` 34/34 PASS, `prove:t9.15g-write-read-restart` 53/53 PASS, `prove:t9.14-reverse-mapper` 19/19 PASS, `smoke` 24/24 PASS, `smoke:meta:canary` 41/41 PASS, `smoke:core:semantic-objectives` 19/19 PASS, `smoke:core:text-extractor` 81/81 PASS; zero diff fora do escopo: zero `src/core/engine.ts`, zero `src/llm/`, zero `panel-nextjs/`, zero wrangler.toml, zero flags, zero Supabase schema/RLS/migrations, zero SYSTEM_PROMPT; **próxima ação T9: Repetir T9.15B-PROVA-REAL-CANARY com facts acumulados entre turnos**.

**T9.15G-FIX-STAGE-AFTER-PERSISTENCE IMPLEMENTADA** (2026-05-04) — PR-FIX contratual; frente T9; causa raiz do loop discovery resolvida: `mapStageCurrentToFaseConversa` retornava `null` para stages pré-docs (`qualification_civil`, `qualification_renda`, `qualification_eligibility`, `discovery`) → `fase_conversa` nunca era gravada no Supabase → banco mantinha default `'inicio'` → read path convertia `inicio` → `discovery` → após restart Worker writeBuffer sumia e funil reiniciava em `discovery`; **fix em `src/supabase/crm-store.ts`**: (1) `mapStageCurrentToFaseConversa` expandida — pré-docs agora gravados com nome canônico (`qualification_civil → 'qualification_civil'`, `qualification_renda → 'qualification_renda'`, `qualification_eligibility → 'qualification_eligibility'`, `discovery → 'discovery'`); pós-docs preservados inalterados (`docs_prep → 'envio_docs'` etc.); (2) `mapFaseConversaToStageCurrent` expandida — lê de volta os mesmos canônicos pré-docs para round-trip completo; fallback legado `inicio → discovery` e valores desconhecidos → `discovery` preservados; (3) docstrings atualizados; **provas atualizadas**: `write-read-restart-proof.ts` — Bloco A (12), B (+4=15), C (+1=10), D (+3=12), E (4) = **53 PASS | 0 FAIL | 1 SKIP** (era 44/44); `reverse-mapper-proof.ts` — Bloco A dividido em A (pré-docs canônicos 4) + A2 (fallback legado 7) + B/C existentes = **19 PASS | 0 FAIL** (era 15/15); **testes**: `prove:t9.15g-write-read-restart` 53/53 PASS, `prove:t9.14-reverse-mapper` 19/19 PASS, `smoke` 24/24 PASS, `smoke:meta:canary` 41/41 PASS, `smoke:core:semantic-objectives` 19/19 PASS; zero diff fora do escopo: zero `src/core/`, zero `src/llm/`, zero `src/meta/outbound.ts`, zero `panel-nextjs/`, zero wrangler.toml, zero flags, zero Supabase schema/RLS/migrations, zero SYSTEM_PROMPT; **próxima ação T9: Repetir T9.15B-PROVA-REAL-CANARY com stage_after persistente**.

**T9.15F-FIX-NEXT-OBJECTIVES-SEMANTICOS IMPLEMENTADA** (2026-05-04) — PR-FIX contratual; frente T9; mapper semântico de `next_objective` criado e aplicado; causa raiz confirmada: `coreDecision.next_objective` era entregue diretamente ao LLM como código opaco (ex: `coletar_customer_goal`, `explicar_mcmv_e_coletar_nome_completo`, `perguntar_nacionalidade`, `perguntar_rnm_e_validade`, `coletar_estado_civil`, `coletar_processo`, `coletar_regime_trabalho`, `coletar_renda_principal`, `avancar_para_qualification_renda`) — LLM recebia código interno e não sabia o que perguntar, resultando em resposta genérica "Olá Bruno! Como posso te ajudar hoje?" em vez de perguntar nacionalidade após nome_completo; **arquivos criados**: (1) `src/core/semantic-next-objective.ts` — mapper puro `toSemanticNextObjective(code: string): string` com 11 mapeamentos canônicos + fallback para código original; (2) `src/core/semantic-next-objective-smoke.ts` — smoke 19/19 PASS (12 unitários + 7 rota realista Core→mapper→semântico); **arquivos modificados**: (3) `src/meta/canary-pipeline.ts` — import + aplicação do mapper em `llmContext.next_objective = toSemanticNextObjective(coreDecision.next_objective)`; (4) `package.json` — `smoke:core:semantic-objectives` adicionado; **testes**: `smoke:core:semantic-objectives` 19/19 PASS, `smoke` 24/24 PASS, `smoke:core:text-extractor` 81/81 PASS, `smoke:meta:canary` 41/41 PASS, `prove:t9.14-reverse-mapper` 15/15 PASS, `prove:t9.15-write-read-restart` 44/44 PASS | 1 SKIP; zero diff fora do escopo: zero `src/llm/`, zero `src/meta/outbound.ts`, zero `src/supabase/`, zero `panel-nextjs/`, zero Supabase schema/RLS/migrations, zero wrangler.toml, zero flags, zero SYSTEM_PROMPT; **próxima ação T9: Repetir T9.15B-PROVA-REAL-CANARY com next_objectives semânticos**.

**T9.15E-IMPL-TOPO-CANONICO-ROUTE IMPLEMENTADA** (2026-05-04) — PR-IMPL contratual; frente T9; rota canônica do topo do funil restaurada cirurgicamente em 5 arquivos do Core Mecânico 2; causa raiz resolvida: `topo-rules.ts`/`topo-gates.ts` avançavam de `discovery` direto para `qualification_civil` ao detectar apenas `customer_goal`, pulando coleta obrigatória de `nome_completo` e `nacionalidade`; rota canônica agora: `customer_goal` → `nome_completo` → `nacionalidade` → (RNM se estrangeiro) → `qualification_civil`; arquivos modificados: (1) `src/core/topo-rules.ts` — `TOPO_REQUIRED_FACTS` expandido para 3 facts obrigatórios; 4 `TOPO_BLOCKING_CONDITIONS`, 4 `TOPO_ADVANCE_CRITERIA`, 5 `TOPO_NEXT_OBJECTIVES` definidos; (2) `src/core/topo-parser.ts` — `TopoSignals` expandido com `nome_completo_detected`, `nome_completo_value`, `nacionalidade_detected`, `nacionalidade_value`, `rnm_detectado`, `rnm_valido`; `extractNomeCompletoCandidato` e `normalizeNacionalidade` e `resolveRnmValido` adicionados; (3) `src/core/topo-gates.ts` — `TopoCriteriaResult` recebe `next_objective`; `evaluateTopoCriteria` reescrito com 4 gates sequenciais; (4) `src/core/engine.ts` — `runTopoDecision` usa `topoCriteria.next_objective` direto; (5) `src/core/text-extractor.ts` — `extractDiscovery` expandido: extrai `nome_completo` (heurística conservadora), `nacionalidade` e `rnm_valido`; negação checada ANTES de positivo; (6) `src/core/smoke.ts` — Cenário 2 atualizado (customer_goal sem nome bloqueia), Cenário 19 atualizado (inclui nome_completo+nacionalidade), Cenários 21-24 adicionados; (7) `src/core/text-extractor-smoke.ts` — 12 novos casos D-N1..D-N6, D-E1..D-E4, D-R1..D-R2; testes: `smoke` 24/24 PASS, `smoke:core:text-extractor` 81/81 PASS, `smoke:meta:canary` 41/41 PASS, `prove:t9.14-reverse-mapper` 15/15 PASS, `prove:t9.15-write-read-restart` 44/44 PASS | 1 SKIP; zero diff fora do escopo permitido; zero diff em `panel-nextjs/`; zero diff em `src/llm/`; zero diff em `src/meta/`; zero diff em `src/supabase/`; **próxima ação T9: Repetir T9.15B-PROVA-REAL-CANARY com rota canônica do topo**.

**FIX/WRANGLER-NO-VARS-OVERWRITE IMPLEMENTADA** (2026-05-04) -- PR-FIX hotfix config/deploy; bloqueante para T9.15B real; wrangler.toml corrigido: keep_vars=true adicionado + 7 flags PROD/canary removidas da secao [vars] top-level (ENOVA2_ENABLED, LLM_REAL_ENABLED, OUTBOUND_CANARY_ENABLED, OUTBOUND_CANARY_WA_ID, SUPABASE_REAL_ENABLED, SUPABASE_WRITE_ENABLED, CANARY_PERCENT); flags de seguranca invariantes mantidas no arquivo (CLIENT_REAL_ENABLED=false, MEMORY_SUPABASE_ENABLED=false, ROLLBACK_FLAG=false, MAINTENANCE_MODE=false, GOLIVE_HARNESS_ENABLED=false); [env.test.vars] mantido intacto; zero diff em src/; zero diff em panel-nextjs/; zero secrets no repo; causa raiz: deploy sobrescrevia flags PROD/canary com defaults false, matando canary e silenciando Enova; fix: keep_vars=true preserva vars do painel nao listadas no arquivo + vars removidas nao sao mais enviadas no deploy; proxima acao: deploy com wrangler.toml corrigido e repetir T9.15B-PROVA-REAL-CANARY com flags PROD preservadas.

**T9.15C-FIX-PARSER-TOPO-FUNIL IMPLEMENTADA** (2026-05-04) — PR-IMPL contratual; correcao cirurgica em src/core/text-extractor.ts (funcao extractDiscovery); causa raiz da falha T9.15B resolvida: compro sozinho e variantes naturais agora detectados como customer_goal=comprar_imovel; guarda de negacao adicionada para nao quero comprar; vocabulario expandido: compro sozinho/a, vou comprar, pretendo comprar, gostaria de comprar, tenho interesse; smoke:core:text-extractor 69/69 PASS (era 56); smoke:meta:canary 41/41 PASS; prove:t9.15-write-read-restart 44/44 PASS; prove:t9.14-reverse-mapper 15/15 PASS; zero diff fora do escopo permitido; G9 permanece aberto; proxima acao T9: Repetir T9.15B-PROVA-REAL-CANARY.

**T9.15B-PROVA-REAL-CANARY CRIADA — ROTEIRO CANARY REAL PRONTO PARA VASQUES** (2026-05-04) — PR-PROVA contratual; DOCS-ONLY; roteiro operacional completo da prova real canary criado em `schema/proofs/T9_15B_REAL_CANARY_FUNNEL_PROOF.md`; pré-condições: `LLM_REAL_ENABLED=true`, `OUTBOUND_CANARY_ENABLED=true`, `OUTBOUND_CANARY_WA_ID=<wa_id>`, `SUPABASE_REAL_ENABLED=true`, `SUPABASE_WRITE_ENABLED=true` — `CLIENT_REAL_ENABLED=false` (não liberar); roteiro 3 turnos: Turno 1 (discovery — intenção MCMV), Turno 2 (qualification_civil — estado civil), Turno 3 (qualification_renda — renda/regime); comandos SQL seguros para verificar `enova_log`, `enova_state`, `crm_lead_meta`; grep no tail salvo para correlação; checklist 15 itens (PIPELINE-01..05, SB-01..05, FUNIL-01..04, TEL-01..03) + painel `/conversations`; critérios G9 impactados: G9-03/G9-07/G9-09/G9-10 com roteiro pronto — pendentes execução Vasques; G9-01/G9-06/G9-08 provados por smoke/health; G9-02/G9-04 provados logicamente (T9.15) — pendentes prova real pós-restart; G9-05 provado (smoke 48/48); G9 permanece aberto; próxima ação: Vasques executa roteiro → registra evidências → T9.R-CLOSEOUT-G9 se tudo passar; zero diff em `panel-nextjs/`; zero diff em `src/core/`, `src/llm/`, `src/meta/outbound`; zero `wrangler.toml`; zero migration/schema/RLS; zero flags; zero `CLIENT_REAL_ENABLED`; **próxima ação T9: Vasques executa roteiro canary e confirma evidências para T9.R-CLOSEOUT-G9**.

**T9.15-PROVA CRIADA — WRITE/READ/RESTART LÓGICO 44/44 PASS** (2026-05-04) — PR-PROVA contratual; `src/supabase/write-read-restart-proof.ts` criado; `prove:t9.15-write-read-restart` adicionado ao `package.json`; 44/44 PASS (Bloco A: 12 write path checks, Bloco B: 11 read path checks, Bloco C: 9 round-trip bidirecional checks, Bloco D: 8 restart lógico simulando rows enova_state pós-restart, Bloco E: 4 SupabaseCrmBackend local writeBuffer); 1 SKIP controlado (Bloco F — prova Supabase real exige SUPABASE_T915_REAL_ENABLED=true + envs reais); regressões: `prove:t9.14-reverse-mapper` 15/15 PASS, `smoke:supabase` 70/70 PASS, `smoke:meta:canary` 41/41 PASS; zero diff em panel-nextjs/; zero migration/schema/RLS; zero wrangler.toml; zero flags de rollout; zero src/core/ src/llm/ alterados; G9-02/G9-04 provados logicamente (round-trip + restart lógico) — prova Supabase real (Bloco F) exige Vasques executar com env real; G9-03/G9-07/G9-09/G9-10 continuam pendentes (conversa real multi-turno); G9 permanece aberto; **próxima ação T9: T9.16-PROVA-CONVERSA-REAL ou Vasques executar Bloco F real**.

**T9.14-IMPL CONCLUÍDA — BLK-T9.14-READ-PATH RESOLVIDO EM CÓDIGO** (2026-05-04) — PR-IMPL contratual; `mapFaseConversaToStageCurrent()` criada e exportada em `src/supabase/crm-store.ts` — reverse mapper puro fase_conversa→stage_current; mapeamento: `envio_docs→docs_prep`, `aguardando_retorno_correspondente→analysis_waiting`, `agendamento_visita→visit_scheduling`, `visita_confirmada→visit_confirmed`, `finalizacao_processo→finalization`, `null/''/'inicio'/desconhecido→discovery`; `mapLeadStateFromEnovaState` corrigida — removida dependência de `row.stage_current` (coluna inexistente — PGRST204 T9.13G), lê `row.fase_conversa` via reverse mapper; `src/supabase/reverse-mapper-proof.ts` criado — 15/15 PASS (Bloco A: 7 valores pré-docs, Bloco B: 5 valores pós-docs, Bloco C: 3 provas de read path sem stage_current); `package.json`: `prove:t9.14-reverse-mapper` adicionado; regressões: `smoke:supabase` 70/70 PASS, `smoke:meta:canary` 41/41 PASS, `prove:g8-readiness` G8 APROVADO; zero diff em panel-nextjs/; zero migration/schema/RLS alterado; zero `wrangler.toml`; zero flags de rollout; zero src/core/ src/llm/ alterados; BLK-T9.14-READ-PATH: **RESOLVIDO EM CÓDIGO** — read path não depende mais de coluna inexistente; G9-02 e G9-04 desbloqueados em código — prova real pós-restart ainda exige conversa com Supabase PROD ativo; **próxima ação T9: T9.15-PROVA ou equivalente para provar write/read/restart e conversa real multi-turno com Supabase PROD**.

**T9.14-DIAG CONCLUÍDA — READ PATH STAGE_CURRENT DIAGNOSTICADO** (2026-05-04) — PR-DIAG pura; DOCS-ONLY; zero src/; zero panel-nextjs/; zero Supabase; diagnóstico formal do bug de read path criado em `schema/diagnostics/T9_14_READ_PATH_STAGE_MAPPING_DIAG.md`; bug identificado: `mapLeadStateFromEnovaState` (src/supabase/crm-store.ts:114) lê `row.stage_current` que não existe no schema real de `enova_state` (PGRST204 confirmado em T9.13G) — resultado: `undefined → asString('') → '' || 'discovery' → 'discovery'` sempre; write path OK (T9.13M-FIX); read path NUNCA implementou reverse mapper `fase_conversa → stage_current`; bloqueio registrado: `BLK-T9.14-READ-PATH`; critérios G9 bloqueados: G9-02 (stage_current gravado/lido em Supabase real) + G9-04 (restart preserva stage); critérios G9 não provados: G9-03/G9-07/G9-09/G9-10; o que T10 provou: painel/CRM/bases/enova_log/health/thread; o que T10 NÃO provou: stage persistente, restart preservando stage, conversa real multi-turno, G9; escopo da próxima IMPL delimitado: somente reverse mapper em src/supabase/crm-store.ts; itens opcionais marcados (não mexer agora); **próxima ação T9: T9.14-IMPL (somente reverse mapper / read path)**.

**T10.7-READINESS CONCLUIDA — T10 PANEL/CRM ENCERRADA POR PROVA** (2026-05-04) — PR-PROVA/CLOSEOUT contratual; DOCS-ONLY; frente T10 Panel/CRM encerrada formalmente apos validacao real end-to-end por Vasques; 13 PRs (#213-#225 + T10.7) executadas; todos os 7 gates G10.1-G10.7 APROVADOS; evidencias reais: (1) painel publicado em https://enova-2.vercel.app/; (2) /api/health retornou ok=true, db_ok=true, worker_ok=true; (3) /bases e /crm carregando dados reais do Supabase; (4) canary end-to-end validado: WhatsApp real -> Worker -> LLM -> enova_log -> Painel — meta_minimal inbound Oi Enova + DECISION_OUTPUT + SEND_OK recebidos; thread nova real visivel no painel /conversations; (5) 10/10 criterios de aceite cumpridos; flags seguras: CLIENT_REAL_ENABLED=false, CANARY restrito a Vasques; documento de readiness: schema/readiness/T10_7_PANEL_CRM_READINESS_CLOSEOUT.md; contrato T10 encerrado; **T9/G9 permanece separado e aberto — proxima acao: T9.14-IMPL**.

**T10.6E-IMPL CONCLUÍDA — WORKER PERSISTE MENSAGENS WHATSAPP EM enova_log** (2026-05-04) — PR-IMPL contratual; `writeEnovaLog()` criada em `src/supabase/crm-store.ts` — função autônoma, fire-safe, usa `supabaseInsert` com service role, retorna skip silencioso quando Supabase não configurado, nunca lança exceção; `EnovaLogEntry` interface criada em `src/supabase/types.ts`; 3 chamadas inseridas em `src/meta/canary-pipeline.ts`: (1) `meta_minimal` após `runInboundPipeline` (inbound — wa_id, meta_type, meta_text, meta_message_id, details.source=enova2/direction=in), (2) `DECISION_OUTPUT` antes de `sendMetaOutbound` (outbound decision — meta_text=replyText, stage, next_stage), (3) `SEND_OK` após outbound.external_dispatch=true (outbound ACK — details.payload_enviado.text.body, provider_message_id, status); smoke `npm run smoke:meta:canary` **41/41 PASS** — zero regressão; build `wrangler deploy --dry-run` PASS (294.20 KiB); zero diff em `panel-nextjs/`; zero migration/schema; tags usados são exatamente os 3 lidos pelo painel (`meta_minimal`, `DECISION_OUTPUT`, `SEND_OK`); prova: `schema/proofs/T10_6E_WORKER_ENOVA_LOG_PERSISTENCE_PROOF.md`; lacunas: LAC-T10.6E-01 validação real pós-deploy (não bloqueante), LAC-T10.6E-02 sem unique constraint em meta_message_id; **próxima ação T10**: T10.7-READINESS. **T9/G9 permanece separado e não é fechado por esta PR**.

**T10.6D-DIAG CRIADA — MAPA PERSISTÊNCIA MENSAGENS ENOVA 1 CONCLUÍDO** (2026-05-04) — PR-DIAG READ-ONLY; repo legado `D:\Enova` lido como fonte somente leitura (zero alterações em D:\Enova); mapa completo Meta → Worker → Supabase → Painel do Enova 1 documentado em `schema/diagnostics/T10_6D_ENOVA1_MESSAGE_PERSISTENCE_MAP.md`; achados: (1) Enova 1 usa tabela única `enova_log` com 3 tags canônicas — `meta_minimal` (inbound, direction=in, campo `meta_text`), `DECISION_OUTPUT` (outbound decision, direction=out, campo `meta_text`), `SEND_OK` (outbound ACK, direction=out, campo `details.payload_enviado.text.body`); (2) persistência via função `logger(env, data)` → POST `/rest/v1/enova_log` com service role; (3) painel E2 (`panel-nextjs/app/api/messages/route.ts`) **já é idêntico** ao painel E1 — lê os mesmos 3 tags, monta direction igual, deduplica DECISION_OUTPUT+SEND_OK igual; (4) menor patch possível identificado: adicionar `writeEnovaLog()` em `src/supabase/crm-store.ts` + 3 chamadas em `src/meta/canary-pipeline.ts` (inbound, pre-outbound, post-outbound) — zero mudança no painel necessária; (5) risco de nova tabela DESCARTADO — `enova_log` já existe, backward compatible, painel já a lê; pré-requisitos da PR-IMPL Worker: Vasques confirma schema de `enova_log` (SELECT column_name, is_nullable...) + confirma RLS/INSERT permission; zero `panel-nextjs/` alterado; zero `src/` alterado; zero Supabase schema; zero D:\Enova alterado; **próxima ação T10**: T10.7-READINESS; **próxima ação Worker (requer autorização Vasques)**: PR-IMPL para gravar em `enova_log` com tags E1. **T9/G9 permanece separado e não é fechado por esta PR**.

**T10.6-CRM-LINK CONCLUÍDA — CRM LINKADO AO SUPABASE REAL + FIX DE MODAIS APLICADO** (2026-05-04) — PR-DIAGFIX contratual; fluxo completo `/crm` UI → Server Action → `crm_leads_v1`/`crm_lead_meta` mapeado; view `crm_leads_v1` confirmada existente e retornando dados reais (Vasques confirmou "Pasta incompleta: 1 / Total: 1 lead"); CRM NÃO usa `enova_attendance_v1` para listagem — usa `crm_leads_v1` com campos E2 canônicos; auth alinhado via `get-admin-key.ts` (`CRM_ADMIN_KEY ?? ENOVA_ADMIN_KEY`); **bug crítico identificado e corrigido**: 5 modais enviavam action names em português (`"atualizar_analise"`, `"marcar_aprovado"`, `"marcar_reprovado"`, `"atualizar_visita"`, `"atualizar_score"`) → backend esperava inglês (`"update_analysis"`, `"update_approved"`, `"update_rejection"`, `"update_visit"`, `"update_analysis"`) → todas as ações falhavam com UNKNOWN_ACTION; field names também remapeados (pt → en) em todos os 5 modais; fix em `panel-nextjs/app/crm/CrmUI.tsx` apenas; `npm run build` PASS 25/25 páginas, zero erros; zero `src/` alterado; zero Supabase schema; diagnóstico: `schema/diagnostics/T10_6_CRM_LINK_SUPABASE_REAL_DIAG.md`; prova: `schema/proofs/T10_6_CRM_LINK_SUPABASE_REAL_FIX_PROOF.md`; **G10.6 APROVÁVEL mediante validação visual Vasques das ações dos modais**; **próxima ação T10**: T10.7-READINESS. **T9/G9 permanece separado e não é fechado por esta PR**.

**T10.6C-DIAG CRIADA — THREAD DE MENSAGENS WHATSAPP DIAGNOSTICADA** (2026-05-04) — PR-DIAG READ-ONLY; causa raiz definitivamente confirmada: Worker Enova-2 NUNCA escreve `enova_log` com nenhuma tag (zero ocorrências em grep de `src/`); `crm_turns` (única tabela CRM que armazena `raw_input_summary` do inbound — truncado a 200 chars) está SEMPRE no `writeBuffer` in-memory — jamais persiste no Supabase real (`src/supabase/crm-store.ts:401` comentário explícito: "crm_turns, crm_facts e demais: fallthrough para writeBuffer"); `/api/messages` (panel-nextjs) lê `enova_log` com tags `meta_minimal`/`DECISION_OUTPUT`/`SEND_OK` — tags escritos EXCLUSIVAMENTE pelo Enova-1; Fix condicional avaliado: TODAS as 5 condições são FALSE → Fix NÃO implementado, diagnóstico apenas; LAC-T10.6C-01 registrada: nenhuma fonte Supabase real existe com texto de mensagem atual + wa_id + direction + created_at; documentado: para ter thread de mensagens real seria necessário uma PR de Worker separada que persista `crm_turns` no Supabase real; `schema/diagnostics/T10_6C_CURRENT_WHATSAPP_MESSAGES_DIAG.md` criado (15 seções); zero `panel-nextjs/` alterado; zero `src/` alterado; zero Supabase schema; próxima ação T10: T10.6-CRM-LINK (ligar CRM real com Supabase; validar views) — G10.6 permanece ABERTO. **T9/G9 permanece separado e não é fechado por esta PR**.

**T10.6B-FIX IMPLEMENTADA — CONVERSAS ALINHADAS COM FONTES ATUAIS ENOVA-2** (2026-05-04) — PR-IMPL contratual; `panel-nextjs/app/api/conversations/route.ts` atualizado: enrichment paralelo de `crm_lead_meta` por `wa_id` adicionado (novos campos: `lead_pool`, `lead_temp`, `status_operacional`, `nome` preferencial); `normalizeFaseConversa()` implementada — whitelist E2 com 5 stages conhecidos (`envio_docs`, `aguardando_retorno_correspondente`, `agendamento_visita`, `visita_confirmada`, `finalizacao_processo`); todos os stages E1 suprimidos silenciosamente; `funil_status` de `enova_state` suprimido inteiramente (sempre null); `panel-nextjs/app/conversations/ConversationUI.tsx` atualizado: tipo `Conversation` expandido, badges E2 (`lead_pool`, `lead_temp`, `status_operacional`) adicionados, badge `funil_status` removido do JSX; `npm run build` PASS — ✓ Compiled successfully, 25 rotas incluindo `/api/conversations` (Dynamic) e `/conversations` (Static); zero TypeScript errors; zero erros de lint; zero src/ alterado; zero Supabase schema; zero segredo commitado; prova criada: `schema/proofs/T10_6B_CONVERSATIONS_CURRENT_SOURCES_PROOF.md`; **lacuna controlada**: LAC-T10.6B-01 — thread de mensagens continua usando `enova_log` E1 (vazia para leads E2 puros — comportamento esperado; frente futura); **próxima ação T10**: T10.6-CRM-LINK (ligar CRM real com Supabase; validar views); G10.6 permanece ABERTO. **T9/G9 permanece separado e não é fechado por esta PR**.
