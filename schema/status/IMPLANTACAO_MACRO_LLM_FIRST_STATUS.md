...
Gate atual: **G9 — ABERTO** — contrato T9 aberto 2026-05-01; PR T9.1 CONCLUÍDA; próxima PR autorizada: T9.2 (Fallback guard com telemetria explícita).

**Estado T9 (2026-05-02):** PR-T9.0 (contrato executivo) concluída. **PR-T9.1 CONCLUÍDA** — `wrangler.toml` atualizado com bloco `[vars]` + `[env.test.vars]` (12 vars com defaults seguros; 8 secrets documentados); `src/runtime/env-validator.ts` criado (20 envs canônicas, separação var/secret, `validateEnvs`, `getPersistenceMode`, nunca vaza valores); `src/runtime/env-smoke.ts` criado; `smoke:runtime:env` **53/53 PASS**; regressões: `smoke:meta:canary` 41/41, `smoke:meta:webhook` 20/20, `smoke:meta:pipeline` 26/26, `prove:g8-readiness` 7/7 PASS. Zero src/meta, zero src/core, zero src/llm, zero src/supabase alterados. **Próxima PR autorizada: T9.2 — Fallback guard com telemetria explícita (`src/crm/store.ts`).**
