...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA + PROVADA (PR #168 + PR #169); **PR-T8.17 CONCLUÍDA** — LLM + outbound canary implementado, gated por flags, `smoke:meta:canary` 41/41 PASS; **próxima PR autorizada: PR-PROVA T8.17** — prova real canary com Vasques (LLM_REAL_ENABLED=true + OUTBOUND_CANARY_ENABLED=true + OUTBOUND_CANARY_WA_ID=wa_id_vasques + OPENAI_API_KEY).
