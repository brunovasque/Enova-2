...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA + PROVADA (PR #168 + PR #169); PR-T8.17 CONCLUÍDA (PR #170); **PR-PROVA T8.17 CONCLUÍDA** (PR #171) — 54 PASS | 0 FAIL | 0 SKIP real positivo, Enova 2 respondeu WhatsApp no número canary, nenhum outro WA recebeu; **PR-T8.18 EM EXECUÇÃO** — runbook de cutover PROD documentado (`schema/operations/T8_CUTOVER_ENOVA2_PROD.md`) + checklist operacional (`schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md`); endpoint PROD confirmado via `wrangler.toml`: `https://nv-enova-2.brunovasque.workers.dev/__meta__/webhook`; rollback preferencial = `ROLLBACK_FLAG=true` (por flags, não webhook); retorno à Enova 1 apenas emergência extrema; **cutover NÃO executado ainda** — aguarda janela operacional com Vasques; G8 NÃO FECHADO.

**Estado Meta/WhatsApp (2026-05-01):** Worker TEST `nv-enova-2-test` publicado; **prova canary real positiva — 54 PASS | 0 FAIL | 0 SKIP** (PR #171, Vasques executou); Enova 2 respondeu WhatsApp no número canary; CRM + memória + LLM + outbound provados em TEST; **cutover PROD documentado em PR-T8.18** — runbook `schema/operations/T8_CUTOVER_ENOVA2_PROD.md`, checklist `schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md`; endpoint PROD confirmado via `wrangler.toml` (`name = "nv-enova-2"`): `https://nv-enova-2.brunovasque.workers.dev/__meta__/webhook`; rollback preferencial = `ROLLBACK_FLAG=true` (flags, não webhook); retorno à Enova 1 apenas emergência extrema; **cutover NÃO executado** — aguarda janela operacional com Vasques; G8 **NÃO FECHADO**. Roadmap oficial: `schema/implementation/T8_ROADMAP_PRODUCAO_WHATSAPP.md`.
