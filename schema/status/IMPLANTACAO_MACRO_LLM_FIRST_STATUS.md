...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA + PROVADA (PR #168 + PR #169); **PR-T8.17 CONCLUÍDA** (PR #170) — LLM + outbound canary implementado; **PR-PROVA T8.17 CONCLUÍDA** (PR #171) — 54 PASS | 0 FAIL | 0 SKIP real positivo, Enova 2 respondeu WhatsApp; **PR-T8.18 EM EXECUÇÃO** — runbook de cutover PROD documentado (`schema/operations/T8_CUTOVER_ENOVA2_PROD.md`) + checklist operacional (`schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md`); endpoint PROD confirmado: `https://nv-enova-2.brunovasque.workers.dev/__meta__/webhook`; **cutover aguarda janela operacional com Vasques**; rollback preferencial por flags; G8 NÃO FECHADO.

**Estado Meta/WhatsApp (2026-05-01):** Worker TEST `nv-enova-2-test` publicado; prova canary real positiva — **54 PASS | 0 FAIL | 0 SKIP** (PR #171); Enova 2 respondeu WhatsApp no número canary; CRM + memória + LLM + outbound provados em TEST; **cutover PROD documentado em PR-T8.18** — runbook `schema/operations/T8_CUTOVER_ENOVA2_PROD.md`, checklist `schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md`; endpoint PROD confirmado via `wrangler.toml`: `https://nv-enova-2.brunovasque.workers.dev/__meta__/webhook`; rollback preferencial = `ROLLBACK_FLAG=true` (por flags, não webhook); retorno à Enova 1 apenas emergência extrema; **cutover NÃO executado ainda** — aguarda janela operacional com Vasques; G8 **NÃO FECHADO**. Roadmap oficial: `schema/implementation/T8_ROADMAP_PRODUCAO_WHATSAPP.md`.
...
**Próxima ação obrigatória (exclusiva de Vasques):** PR-T8.12B — provisionar 4 secrets Meta (`META_VERIFY_TOKEN`, `META_APP_SECRET`, `META_ACCESS_TOKEN`, `META_PHONE_NUMBER_ID`), publicar Worker test (`wrangler deploy --env test`), registrar webhook Meta, executar `prove:meta-controlada` com `META_REAL_ENABLED=true`, documentar evidência de challenge real + inbound real + logs Cloudflare. Após isso: autorizar `CLIENT_REAL_ENABLED=true` + `LLM_REAL_ENABLED=true` + go-live.

PR-T8.18 em execução em 2026-05-01 — **Cutover controlado Enova 2 em produção WhatsApp** (PR-OPS/GO-LIVE CONTROLADO); `schema/operations/T8_CUTOVER_ENOVA2_PROD.md` criado — runbook completo de cutover: endpoint PROD confirmado via `wrangler.toml` (`https://nv-enova-2.brunovasque.workers.dev/__meta__/webhook`), deploy `npx wrangler deploy`, checklist de secrets/flags, 3 fases (A pré-cutover, B cutover, C monitoramento), rollback por 3 níveis (Nível 1: flags/segundos, Nível 2: parcial, Nível 3: emergência extrema/webhook Enova 1); `schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md` criado — checklist operacional por fase (A1..A5, B1..B5, C1, Rollback, D ampliação, E closeout); rollback preferencial documentado como `ROLLBACK_FLAG=true` — retorno à Enova 1 explicitamente emergência extrema; CLIENT_REAL_ENABLED=false para cutover inicial (ampliar na Fase D, decisão de Vasques); T8.12B NÃO ENCERRADA; G8 NÃO FECHADO. **Próxima ação: Vasques executa cutover conforme checklist.**
