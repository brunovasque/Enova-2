...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA + PROVADA (PR #168 + PR #169); PR-T8.17 CONCLUÍDA (PR #170); **PR-PROVA T8.17 CONCLUÍDA** (PR #171) — 54 PASS | 0 FAIL | 0 SKIP real positivo, Enova 2 respondeu WhatsApp no número canary, nenhum outro WA recebeu; **PR-T8.18 MERGEADA** — runbook de cutover PROD documentado (`schema/operations/T8_CUTOVER_ENOVA2_PROD.md`) + checklist operacional (`schema/proofs/T8_CUTOVER_PROD_CHECKLIST.md`); cutover PROD executado: PROD recebeu GET challenge OK + POST webhook OK, mas resposta WhatsApp não chegou — causa desconhecida; **PR-DIAG T8-PROD-TELEMETRIA EM EXECUÇÃO** — 11 logs `console.log` cirúrgicos adicionados ao caminho webhook→CRM→LLM→outbound para diagnóstico via `wrangler tail`; smokes 41/41 + 26/26 + 20/20 PASS; G8 NÃO FECHADO.

**Estado Meta/WhatsApp (2026-05-01):** Worker PROD `nv-enova-2` publicado; cutover executado: PROD recebeu GET challenge OK + POST webhook OK; resposta WhatsApp NÃO chegou — causa em diagnóstico; **PR-DIAG T8-PROD-TELEMETRIA instrumenta 11 pontos** (`meta.prod.webhook.received`, `meta.prod.webhook.signature`, `meta.prod.webhook.parsed`, `meta.prod.dedupe`, `meta.prod.flags.snapshot`, `meta.prod.pipeline.result`, `meta.prod.llm.gate`, `meta.prod.llm.result`, `meta.prod.outbound.gate`, `meta.prod.outbound.result`, `meta.prod.webhook.final`) visíveis via `wrangler tail nv-enova-2`; nenhum comportamento funcional alterado; G8 **NÃO FECHADO**. Roadmap oficial: `schema/implementation/T8_ROADMAP_PRODUCAO_WHATSAPP.md`.
