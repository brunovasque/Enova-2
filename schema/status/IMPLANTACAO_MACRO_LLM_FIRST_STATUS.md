...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA; **PR-PROVA T8.16 em execução** — smokes locais 16/16 PASS, prova real pendente de execução por Vasques (`ENOVA2_PROOF_ENABLED=true` + credenciais + `npm run prove:meta:pipeline-real`); **próxima PR autorizada: PR-PROVA T8.16 (execução por Vasques) → se aprovada, PR-T8.17** (LLM controlado).

**Estado Meta/WhatsApp (2026-05-01):** Worker TEST `nv-enova-2-test` publicado; inbound real recebido (PR #165); diagnóstico do gap concluído (PR #166); **acoplamento inbound → CRM + memória IMPLEMENTADO (PR-T8.16)** — pipeline `src/meta/pipeline.ts` operacional, `upsertLeadByPhone` + `createConversationTurn` + `registerMemoryEvent(source:'meta_webhook')`, gated por `ENOVA2_ENABLED=true`; acoplamento LLM/outbound **NÃO implementado**; Enova 2 **ainda NÃO responde WhatsApp**; cutover Enova 1 → Enova 2 **NÃO executado**; G8 **NÃO FECHADO**. Roadmap oficial: `schema/implementation/T8_ROADMAP_PRODUCAO_WHATSAPP.md`. 1 número WhatsApp = 1 webhook ativo por vez — cutover exige janela controlada com Vasques presente e rollback em ~30s via painel Meta.
