...
Gate atual: **G8 — em aberto** — T8 em execução; PR-T8.16 CONCLUÍDA + PROVADA (PR #168 + PR #169); **PR-T8.17 CONCLUÍDA** (PR #170) — LLM + outbound canary implementado, gated por flags, `smoke:meta:canary` 41/41 PASS; **PR-PROVA T8.17 EM EXECUÇÃO** — harness `src/meta/canary-real-proof.ts` instalado: 31 PASS | 0 FAIL | 5 SKIP (modo local); `schema/proofs/T8_LLM_OUTBOUND_CANARY_PROVA_REAL.md` criado; **prova real aguarda Vasques** com `CANARY_REAL_PROOF_ENABLED=true` + flags canary ativas no Worker TEST (LLM_REAL_ENABLED=true + OUTBOUND_CANARY_ENABLED=true + OUTBOUND_CANARY_WA_ID + OPENAI_API_KEY).

**Estado Meta/WhatsApp (2026-05-01):** Worker TEST `nv-enova-2-test` publicado; inbound real recebido (PR #165); diagnóstico do gap concluído (PR #166); **acoplamento inbound → CRM + memória IMPLEMENTADO (PR-T8.16)** — pipeline `src/meta/pipeline.ts` operacional; **acoplamento LLM + outbound canary IMPLEMENTADO (PR-T8.17)** — `src/meta/canary-pipeline.ts` + `src/llm/client.ts` operacionais, gated por `LLM_REAL_ENABLED` + `OUTBOUND_CANARY_ENABLED` + `OUTBOUND_CANARY_WA_ID`; Enova 2 **ainda NÃO responde WhatsApp em produção** — flags desligadas por padrão; cutover Enova 1 → Enova 2 **NÃO executado**; G8 **NÃO FECHADO**. Roadmap oficial: `schema/implementation/T8_ROADMAP_PRODUCAO_WHATSAPP.md`. 1 número WhatsApp = 1 webhook ativo por vez — cutover exige janela controlada com Vasques presente e rollback em ~30s via painel Meta.
