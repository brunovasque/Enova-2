...
Próxima PR autorizada: **PR-T8.9B — Execução real Supabase com env real controlado (PR-PROVA)** — rodar `prove:supabase-real` com `SUPABASE_REAL_ENABLED=true` + `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` e anexar evidência positiva de leitura real.
