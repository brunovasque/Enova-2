...
PR-T8.12 concluída: `src/meta/proof-controlled.ts` + `schema/proofs/T8_META_WHATSAPP_PROVA_CONTROLADA.md` + `package.json` (`prove:meta-controlada`) criados em 2026-04-30 — **Prova Meta/WhatsApp controlada** (PR-PROVA); script dual-mode instalado — sem `META_REAL_ENABLED`: 25 PASS | 0 FAIL | 6 SKIP exit 0 (nunca falha CI); P1 smokes unitários 100% PASS (signature HMAC, parser, dedupe, outbound, challenge, POST processado); P2–P7 SKIPPED por bloqueio controlado (secrets Meta reais ausentes no ambiente Claude Code — `META_VERIFY_TOKEN`, `META_APP_SECRET`, `META_ACCESS_TOKEN`, `META_PHONE_NUMBER_ID` não provisionados no Worker); P8 invariantes soberania 6/6 PASS (parser nunca produz `reply_text`/`stage`/`speech_intent`; outbound bloqueado por `CHANNEL_ENABLED=false`/`META_OUTBOUND_ENABLED=false`); `prove:meta-controlada` registrado no `package.json` fora de `smoke:all`; instrução de ativação da prova real documentada em §7 do artefato de prova; zero cliente real, zero outbound real, zero WhatsApp real amplo; **frente Meta/WhatsApp estado: IMPLEMENTADO_PROVA_PARCIAL** — bloqueio real por secrets + deployment (externo ao código).

Próxima PR autorizada: **PR-T8.13 — Memória evolutiva + telemetria operacional** (PR-IMPL) — dependente de PR-T8.10 + PR-T8.11 concluídas.
