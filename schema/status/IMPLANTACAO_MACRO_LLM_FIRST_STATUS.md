...
PR-T8.9B em execução: `src/supabase/proof.ts` atualizado com bloco P0 de diagnóstico de rede + `extractNetworkCause()`; duas execuções reais por Vasques registradas: rodada 1 (2/8 PASS — `network_error: fetch failed` por bloqueio de rede local, resolvido pelo Vasques); rodada 2 (7/8 PASS — P2 auth 401 OK, P3 `crm_lead_meta` OK `rows=6`, P5 dossier OK, P6 `enova_document_files` OK `rows=0`, P7 storage buckets OK `found=4/4`, P8 SKIPPED correto, **P4 `enova_docs` FAIL**: `column enova_docs.updated_at does not exist` — coluna real é `created_at`); **duas correções aplicadas**: (1) `proof.ts` P4: `updated_at.desc` → `created_at.desc`; (2) `crm-store.ts:195` `readDocuments()`: `updated_at.desc` → `created_at.desc` (mesmo bug no Worker runtime — corrigido no mesmo commit); verificação completa de outras ocorrências: `types.ts`, `smoke.ts`, `proof.ts:271` e `crm-store.ts:130` — todos seguros (fallback gracioso ou tabela diferente com coluna existente); `schema/implementation/T8_SUPABASE_PROVA_REAL_EXECUTADA.md` atualizado; `smoke:supabase` 70/70 PASS, `prove:crm-e2e` 73/73 PASS, `smoke:all` PASS, `prove:supabase-real` skip exit 0. Frente Supabase permanece aberta até P4 passar na reexecução de Vasques.

Próxima PR autorizada: **PR-T8.9B reexecução** — Vasques roda `prove:supabase-real` com harness e runtime corrigidos. Resultado esperado: 8/8 PASS | 1 SKIPPED | 0 FAIL. Se positivo, frente Supabase encerrada e próxima frente definida por Vasques.
