...
ORQUESTRADOR T4
  ...
  │   4c. Executar policy engine T3 sobre o estado pós-LLM proposto            │
  │       (prior_lead_state + ProposedStateDelta reconciliado +                 │
  │        LLMResponseMeta + current_objective) → PolicyDecisionSet pós-LLM    │
  │   4d. Montar ValidationContext com PolicyDecisionSet pós-LLM               │
  │   4e. Executar validador VC-01..VC-09 (T3.4)                               │
  │   4f. Emitir PersistDecision (ValidationResult)                            │
  │   4g. Aplicar PersistDecision → atualizar ou não o lead_state              │
  │   4h. Registrar ValidationResult no validation_log                         │
  │                                                                             │
...
| T4.1 (ContextoTurno) | `prior_decisions` | `PolicyDecisionSet?` — decisões pré-LLM (contexto prévio; não substitui execução T3 em §3.5) |
| T4.3 (executado aqui — §3.5) | `PolicyDecisionSet` pós-LLM | `PolicyDecisionSet` — produzido pelo engine T3 sobre o estado proposto |
...
## §3.5 Execução do policy engine T3 sobre o estado proposto

### 3.5.1 Quando executar

Após reconciliação (§3), o mecânico executa o **pipeline de 6 estágios do policy engine T3**
(T3_ORDEM_AVALIACAO_COMPOSICAO §1) sobre o estado pós-LLM proposto. Esta execução produz o
`PolicyDecisionSet` pós-LLM que será usado na montagem do `ValidationContext` (§4).

### 3.5.2 Entradas para a execução T3

```
Entradas do policy engine T3 em T4.3:
  prior_lead_state + ProposedStateDelta reconciliado   → estado proposto combinado
  LLMResponseMeta                                      → sinais estruturados da resposta
  current_objective                                    → objetivo operacional do turno
  ContextoTurno.prior_decisions (se presente)          → contexto prévio (não substituído)
```

O **estado proposto combinado** é o `prior_lead_state` com as mudanças do `ProposedStateDelta`
reconciliado projetadas provisoriamente — para que o engine T3 avalie o estado resultante das
coletas do LLM, não apenas o estado anterior à chamada LLM.

### 3.5.3 Saída — `PolicyDecisionSet` pós-LLM

```
PolicyDecisionSet {
  decisions:       PolicyDecision[]   — 5 classes: BLOQUEIO, OBRIGAÇÃO, CONFIRMAÇÃO,
                                        SUGESTÃO MANDATÓRIA, ROTEAMENTO
  collisions:      CollisionRecord[]  — colisões entre decisões incompatíveis (RC-INV-03)
  soft_vetos:      VetoSuaveRecord[]  — vetos suaves ativos pós-LLM
  evaluation_meta: EvaluationMeta     — estágio de avaliação, turno, timestamp
}
```

### 3.5.4 Regras da execução T3 em T4.3

**VP-T3-01:** O engine T3 opera sobre o estado proposto estruturado — nunca sobre `reply_text`.
O engine decide ações e guardrails sobre estado e objetivo; fala é escopo exclusivo do LLM.

**VP-T3-02:** `ContextoTurno.prior_decisions` pode existir como contexto prévio (pré-LLM).
**Não substitui** a execução do engine T3 sobre o estado pós-LLM proposto. As duas podem
coexistir; o `ValidationContext` usa o resultado pós-LLM (§3.5.3), não o pré-LLM.

**VP-T3-03:** Toda colisão detectada pelo engine T3 em §3.5 deve constar em
`PolicyDecisionSet.collisions[]`. Colisão silenciosa viola RC-INV-03 e causa VC-04 FAIL → `REJECT`.

**VP-T3-04:** O engine T3 não acessa, não avalia e não produz `reply_text`. Sua saída é
exclusivamente `PolicyDecisionSet`. Soberania de fala permanece com o LLM (A00-ADENDO-01).
...
Após reconciliação (§3) e execução do policy engine T3 sobre o estado proposto (§3.5), o
mecânico monta o `ValidationContext` e o passa ao validador.
...
ValidationContext {
  ...
  policy_decision_set:   PolicyDecisionSet    — resultado do engine T3 executado em §3.5
                                               sobre o estado pós-LLM proposto
...
**VP-VC-03:** `policy_decision_set` é o resultado da **execução do pipeline T3 de 6 estágios**
sobre o estado pós-LLM proposto (§3.5). `ContextoTurno.prior_decisions` pode existir como
contexto prévio — **não substitui** esta execução.
...
- Colisões são detectadas pelo policy engine T3 executado em §3.5 sobre o estado pós-LLM proposto.
- VC-04 verifica que `PolicyDecisionSet.collisions[]` (produzido em §3.5) cobre toda colisão detectável no delta.
- Se VC-04 detecta colisão não registrada no `PolicyDecisionSet` pós-LLM → VC-04 FAIL → `REJECT`.
...
| **VP-INV-10** | T4.3 executa o pipeline de 6 estágios do policy engine T3 (§3.5) sobre o estado pós-LLM proposto: `prior_lead_state + ProposedStateDelta reconciliado + LLMResponseMeta + current_objective`. `ContextoTurno.prior_decisions` é contexto prévio (pré-LLM) — não substitui esta execução. |
...
| **AP-VP-07** | Usar `ContextoTurno.prior_decisions` (pré-LLM) como substituto do `PolicyDecisionSet` pós-LLM — não executar o engine T3 sobre o estado proposto em §3.5 | VP-INV-10; VP-T3-02; VP-VC-03 |
...
| **Microetapa 3 — Integrar policy engine + reconciliador antes de persistir** | **Cobertura completa:** §2 (ProposedStateDelta), §3 (reconciliação T2.4), §3.5 (execução T3 sobre estado proposto → PolicyDecisionSet pós-LLM), §4 (ValidationContext com PolicyDecisionSet pós-LLM), §5 (validador VC-01..09), §6 (PersistDecision), §7 (safe_fields/blocked_fields), §8 (conflitos/colisões), §9 (aplicação), §10 (reply_text não reescrito), §11 (quando atualizar) ✓ |
...
| `T3_ORDEM_AVALIACAO_COMPOSICAO.md §1-§5` | Pipeline 6 estágios executado em T4.3 sobre estado pós-LLM proposto (§3.5) | **PASS** — VP-INV-10, VP-T3-01..04 |
| `T3_ORDEM_AVALIACAO_COMPOSICAO.md §6` | `PolicyDecisionSet` shape com `decisions[]`, `collisions[]`, `soft_vetos[]` | **PASS** — §3.5.3, §4.2, VP-VC-03 |
...
| `A00-ADENDO-02` | Orquestrador como coordenador; T4.3 executa T3 como componente; engine não fala, não reescreve reply_text | **PASS** — VP-T3-04; VP-INV-10; §3.5 |
...
Há lacuna remanescente?:               não —
                                       ...
                                       execução do policy engine T3 sobre estado pós-LLM
                                       proposto (§3.5) com VP-T3-01..04 e PolicyDecisionSet
                                       pós-LLM (prior_lead_state + ProposedStateDelta
                                       reconciliado + LLMResponseMeta + current_objective);
                                       ValidationContext montagem (§4) com PolicyDecisionSet
                                       pós-LLM e LLMResponseMeta;
                                       ...
                                       VP-INV-01..12; VP-T3-01..04; 12 anti-padrões AP-VP;
                                       5 exemplos sintéticos E1–E5;
                                       microetapa 3 coberta; cross-ref T2/T3/T4.1/T4.2 em 19
...
Há item parcial/inconclusivo bloqueante?: não —
                                       T4.3 executa T3 sobre estado proposto (VP-INV-10);
                                       engine T3 não lê nem produz reply_text (VP-T3-04);
