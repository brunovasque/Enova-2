...
|------|--------|--------------------------|
| T9 — Integração LLM ↔ Funil Mecânico ↔ Supabase Real ↔ Telemetria | **aberto** — T9.15H-FIX-FACTS-PERSISTENCE-TOPO implementada em 2026-05-04; bug de perda de facts entre turnos corrigido via `enova_state.last_context`; `readLeadAccumulatedFacts`/`writeLeadAccumulatedFacts` adicionados em `crm-store.ts`; merge facts no `canary-pipeline.ts`; prova 34/34 PASS; smokes 24/24 + 41/41 + 19/19 + 81/81 + 53/53 + 19/19 PASS; zero diff fora do escopo; contrato ativo em `schema/contracts/active/CONTRATO_T9_LLM_FUNIL_SUPABASE_RUNTIME.md` | **Repetir T9.15B-PROVA-REAL-CANARY** com facts acumulados entre turnos |
...
- 2026-05-04 — T9.15H-FIX-FACTS-PERSISTENCE-TOPO implementada: bug crítico de perda de facts entre turnos corrigido sem migration Supabase; causa raiz: `crm_facts` é writeBuffer-only — ao reiniciar o isolate Cloudflare Worker, writeBuffer zera e facts do turno 1 (customer_goal) somem no turno 2; fix: `enova_state.last_context` (coluna confirmada T9.13G P0) usada como storage JSON para facts acumulados; `readLeadAccumulatedFacts` lê last_context (TEXT→parse ou JSONB direto) antes do Core; `writeLeadAccumulatedFacts` escreve facts acumulados após Core; merge `{ ...persistedFacts, ...extractedFacts }` garante precedência do turno atual; prova `facts-persistence-proof.ts` 34/34 PASS (cenários A-G: merge básico, não-sobrescrita, acumulação 3 turnos, restart completo, parse TEXT/JSONB, precedência, topo completo); smokes 24/24 + 41/41 + 19/19 + 81/81 + 53/53 + 19/19 PASS; zero diff fora do escopo; próxima PR: Repetir T9.15B-PROVA-REAL-CANARY com facts acumulados entre turnos.
- 2026-05-04 — T9.15G-FIX-STAGE-AFTER-PERSISTENCE implementada: `mapStageCurrentToFaseConversa` expandida para gravar stages pré-docs (`qualification_civil → 'qualification_civil'` etc.) em `fase_conversa`; `mapFaseConversaToStageCurrent` expandida com round-trip pré-docs; fallback legado `inicio → discovery` preservado; causa raiz do loop discovery resolvida (Supabase não mais mantinha default 'inicio' para pré-docs); smokes 53/53 + 19/19 + 24/24 + 41/41 + 19/19 PASS; zero diff fora do escopo; próxima PR: Repetir T9.15B-PROVA-REAL-CANARY com stage_after persistente.
- 2026-05-04 — T9.15F-FIX-NEXT-OBJECTIVES-SEMANTICOS implementada: mapper semântico `toSemanticNextObjective` criado em `src/core/semantic-next-objective.ts`; aplicado em `canary-pipeline.ts` na montagem de `LlmContext.next_objective`; causa raiz: `coreDecision.next_objective` era código opaco entregue diretamente ao LLM; LLM recebia `perguntar_nacionalidade` (opaco) e respondia genericamente; mapper converte 11 códigos opacos para instruções semânticas em português; fallback para código original quando desconhecido; smokes 19/19 + 24/24 + 81/81 + 41/41 + 15/15 + 44/44 PASS; zero diff fora do escopo; próxima PR: Repetir T9.15B-PROVA-REAL-CANARY com next_objectives semânticos.
- 2026-05-04 — T9.15E-IMPL-TOPO-CANONICO-ROUTE implementada: rota canônica do topo restaurada em 5 arquivos Core Mecânico 2; causa raiz: `discovery` avançava para `qualification_civil` com apenas `customer_goal`, pulando coleta de `nome_completo` e `nacionalidade`; gates sequenciais implementados: customer_goal → nome_completo → nacionalidade → (rnm se estrangeiro) → qualification_civil; `TOPO_REQUIRED_FACTS` expandido; `TOPO_NEXT_OBJECTIVES` com 5 objetivos canônicos; parser L05 + gates L06 + text-extractor + smoke atualizados; smokes 24/24 + 81/81 + 41/41 + 15/15 + 44/44 PASS; zero diff fora do escopo; próxima PR: Repetir T9.15B-PROVA-REAL-CANARY.
