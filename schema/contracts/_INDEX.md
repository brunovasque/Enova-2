...
|------|--------|--------------------------|
| T9 — Integração LLM ↔ Funil Mecânico ↔ Supabase Real ↔ Telemetria | **aberto** — T9.15F-FIX-NEXT-OBJECTIVES-SEMANTICOS implementada em 2026-05-04; mapper semântico `toSemanticNextObjective` criado em `src/core/semantic-next-objective.ts`; aplicado em `canary-pipeline.ts` na montagem de `LlmContext.next_objective`; 11 mapeamentos canônicos (topo + Meio A + Meio B); smokes 19/19 + 24/24 + 81/81 + 41/41 + 15/15 + 44/44 PASS; zero diff em `src/llm/`, `src/meta/outbound.ts`, `src/supabase/`, `panel-nextjs/`, wrangler.toml, SYSTEM_PROMPT; contrato ativo em `schema/contracts/active/CONTRATO_T9_LLM_FUNIL_SUPABASE_RUNTIME.md` | **Repetir T9.15B-PROVA-REAL-CANARY** com next_objectives semânticos (após deploy desta branch) |
