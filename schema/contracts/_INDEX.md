...
|------|--------|--------------------------|
| T9 — Integração LLM ↔ Funil Mecânico ↔ Supabase Real ↔ Telemetria | **aberto** — T9.15E-IMPL-TOPO-CANONICO-ROUTE implementada em 2026-05-04; rota canônica do topo restaurada cirurgicamente em 5 arquivos do Core Mecânico 2 (`topo-rules.ts`, `topo-parser.ts`, `topo-gates.ts`, `engine.ts`, `text-extractor.ts`); gates sequenciais: `customer_goal → nome_completo → nacionalidade → (rnm se estrangeiro) → qualification_civil`; smokes 24/24 + 81/81 + 41/41 + 15/15 + 44/44 PASS; contrato ativo em `schema/contracts/active/CONTRATO_T9_LLM_FUNIL_SUPABASE_RUNTIME.md` | **Repetir T9.15B-PROVA-REAL-CANARY** com rota canônica do topo (após deploy desta branch) |
