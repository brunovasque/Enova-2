...
|------|--------|--------------------------|
| T9 — Integração LLM ↔ Funil Mecânico ↔ Supabase Real ↔ Telemetria | **aberto** — T9.15G-FIX-STAGE-AFTER-PERSISTENCE implementada em 2026-05-04; `mapStageCurrentToFaseConversa` expandida para gravar stages pré-docs canônicos (`qualification_civil`, `qualification_renda`, `qualification_eligibility`, `discovery`) em `fase_conversa`; `mapFaseConversaToStageCurrent` expandida para round-trip completo; causa raiz do loop discovery resolvida; smokes 53/53 + 19/19 + 24/24 + 41/41 + 19/19 PASS; zero diff fora do escopo; contrato ativo em `schema/contracts/active/CONTRATO_T9_LLM_FUNIL_SUPABASE_RUNTIME.md` | **Repetir T9.15B-PROVA-REAL-CANARY** com stage_after persistente |
