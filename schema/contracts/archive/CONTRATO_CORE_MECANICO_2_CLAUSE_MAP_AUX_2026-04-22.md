...
|---|-------|-------|------------------------------------------|---------------------------|------|-----------------|
| L-01 | PDF 1 | L03 | Stages, gates, transições e microregras do funil — referência central de fluxo | Define o mapa de stages e gates que o Core implementa. Bloco mais crítico. | estado estruturado | ✅ EXECUTADO — PR L03 (esqueleto): `src/core/stage-map.ts`, `src/core/engine.ts` |
| L-02 | PDF 1 | L04 | Contrato e regras operacionais do topo do funil | Define regras operacionais do stage de topo. | policy/governança | ✅ EXECUTADO — PR L04+L05+L06: `src/core/topo-rules.ts` |
| L-03 | PDF 1 | L05 | Parser e critérios de extração do topo | Define critérios de extração de sinais do topo. Interface Core ↔ Extractor. | estado estruturado | ✅ EXECUTADO — PR L04+L05+L06: `src/core/topo-parser.ts` |
| L-04 | PDF 1 | L06 | Critérios de aceite e validação do topo | Define gates de validação e aceite no topo. | gate | ✅ EXECUTADO — PR L04+L05+L06: `src/core/topo-gates.ts` |
| L-05 | PDF 1 | L07 | Regras de estado civil e impacto na composição familiar | Define regras de estado civil para Meio A. | policy/governança | ✅ EXECUTADO — PR L07 + L08: `src/core/meio-a-rules.ts`, `src/core/meio-a-parser.ts`, `src/core/meio-a-gates.ts`, `src/core/engine.ts` |
...
| L-13 | PDF 1 | L15 | Trilhos especiais e variantes P3 e multi-proponente | Define trilhos especiais do funil. | estado estruturado | ✅ EXECUTADO — PR L15 + L16: `src/core/especiais-rules.ts`, `src/core/especiais-parser.ts`, `src/core/especiais-gates.ts`, `src/core/engine.ts`, `src/core/smoke.ts`, `src/core/stage-map.ts`, `src/core/types.ts` |
| L-14 | PDF 1 | L16 | Composição familiar especial e variantes de elegibilidade | Complementa L15. | policy/governança | ✅ EXECUTADO — PR L15 + L16: `src/core/especiais-rules.ts`, `src/core/especiais-parser.ts`, `src/core/especiais-gates.ts`, `src/core/engine.ts`, `src/core/smoke.ts`, `src/core/stage-map.ts`, `src/core/types.ts` |
| L-15 | PDF 1 | L17 | Transição final do funil, documentos, handoff e visita | Define a fase final operacional. | estado estruturado | ✅ EXECUTADO — PR L17: `src/core/final-rules.ts`, `src/core/final-parser.ts`, `src/core/final-gates.ts`, `src/core/engine.ts`, `src/core/smoke.ts`, `src/core/stage-map.ts`, `src/core/types.ts`, `src/worker.ts`, `src/worker-route-smoke.ts` |
...
| Ordem | Bloco(s) | Recorte | Pré-requisito | Status |
|-------|----------|---------|---------------|--------|
...
| 5 | L15, L16 | Especiais — trilhos P3, multi, variantes | Meio B funcional (Ordem 4) | ✅ EXECUTADO |
| 6 | L17 | Final — transição final, docs, visita | Especiais funcional (Ordem 5) | ✅ EXECUTADO |
| 7 | Integração | Smoke test de trilho completo (topo → final) | Todos os stages (Ordens 1–6) | ✅ EXECUTADO — PR L17: `src/core/smoke.ts`, `src/worker-route-smoke.ts` |
