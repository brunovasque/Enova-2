# 19 — Conteúdo completo: src/core/meio-b-parser.ts

**Data:** 2026-05-06
**Fonte:** `git show HEAD:src/core/meio-b-parser.ts`
...
### Interface `MeioBSignals` — 14 campos

| Campo | Tipo | Derivado de |
|---|---|---|
| `regime_trabalho_detected` | boolean | `merged['regime_trabalho']` normalizado |
| `regime_trabalho_value` | `RegimeTrabalho\|null` | normalizeRegime |
| `renda_principal_detected` | boolean | `merged['renda_principal']` normalizado |
| `renda_principal_value` | `number\|null` | normalizeRenda |
| `autonomo_tem_ir_detected` | boolean | `merged['autonomo_tem_ir']` |
| `autonomo_tem_ir_value` | `boolean\|null` | normalizeBoolean |
| `ctps_36_detected` | boolean | `merged['ctps_36']` |
| `ctps_36_value` | `boolean\|null` | normalizeBoolean |
| `nacionalidade_detected` | boolean | `merged['nacionalidade']` |
| `nacionalidade_value` | `Nacionalidade\|null` | normalizeNacionalidade |
| `rnm_status_detected` | boolean | `merged['rnm_status']` |
| `rnm_status_value` | `RnmStatus\|null` | normalizeRnmStatus |
| `autonomo_ir_required` | boolean | `regimeValue === 'autonomo'` |
| `low_income_solo_signal` | boolean | `processoValue === 'solo' && rendaValue < 3000` |

### Dependência cross-stage

`low_income_solo_signal` usa `merged['processo']` — fact do Meio A que deve estar presente em `facts_current` via persistência entre turnos. Sem isso, o sinal não dispara mesmo com renda baixa e processo solo.

### `normalizeBoolean` — aceita múltiplos formatos

Valores `true`: `true`, `'sim'`, `'true'`, `'1'`
Valores `false`: `false`, `'nao'`, `'não'`, `'false'`, `'0'`

### `computeParseStatus`

- `'ready'`: `regime_trabalho` E `renda_principal` presentes
- `'partial'`: qualquer um dos 6 sinais detectado
- `'empty'`: nenhum sinal
