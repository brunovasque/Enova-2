# 14 — src/core/topo-rules.ts — Conteúdo Completo

**Arquivo:** `src/core/topo-rules.ts`  
...
### Facts obrigatórios do topo

| Fact | Obrigatório | Descrição |
|---|---|---|
| `customer_goal` | SIM | Interesse do lead — gate 1 |
| `nome_completo` | SIM | Nome coletado após interesse — gate 2 |
| `nacionalidade` | SIM | Brasileiro/estrangeiro/naturalizado — gate 3 |
| `rnm_valido` | Condicional | Só exigido se `nacionalidade='estrangeiro'` — gate 4 |
| `channel_origin` | NÃO | Opcional (F0) |
| `current_intent` | NÃO | Opcional (F7) |

### TOPO_NEXT_OBJECTIVES — mapeamento para SEMANTIC_MAP

| Constante | Código emitido | Mapeado em semantic-next-objective.ts? |
|---|---|---|
| `COLETAR_CUSTOMER_GOAL` | `'coletar_customer_goal'` | SIM |
| `EXPLICAR_MCMV_E_COLETAR_NOME` | `'explicar_mcmv_e_coletar_nome_completo'` | SIM |
| `PERGUNTAR_NACIONALIDADE` | `'perguntar_nacionalidade'` | SIM |
| `PERGUNTAR_RNM` | `'perguntar_rnm_e_validade'` | SIM (atualizado T9.16A) |
| `AVANCAR_PARA_CIVIL` | `'Perguntar estado civil: solteiro, casado, união estável ou divorciado.'` | SIM (mapper normaliza para versão com "(a)") |

**Nota:** `AVANCAR_PARA_CIVIL` já é semi-semântico na origem — é a única constante do topo
que emite texto legível em vez de código opaco. O SEMANTIC_MAP normaliza para incluir "(a)".

### TOPO_BLOCKING_CONDITIONS — códigos de log
...
### TOPO_NEXT_STEP — transições

| Constante | Valor | Quando |
|---|---|---|
| `ADVANCE_TO_QUALIFICATION` | `'qualification_civil'` | Topo mínimo completo |
| `REMAIN_IN_DISCOVERY` | `'discovery'` | Qualquer gate bloqueado |

### Lacunas identificadas

- **LG-01**: `TOPO_OPTIONAL_FACTS` (`channel_origin`, `current_intent`) declarados mas nunca extraídos pelo `text-extractor.ts` na stage `discovery`.
- **LG-02**: Não há constante para encerramento por inelegibilidade RNM — se o cliente tem RNM com prazo determinado, o Core bloqueia indefinidamente em `ESTRANGEIRO_SEM_RNM_VALIDO` sem rota de saída.
- **LG-03**: `AVANCAR_PARA_CIVIL` mistura código e texto legível — inconsistência com os demais objetivos que usam snake_case opaco.
