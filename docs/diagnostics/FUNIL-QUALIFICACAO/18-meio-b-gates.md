# 18 — Conteúdo completo: src/core/meio-b-gates.ts

**Data:** 2026-05-06
**Fonte:** `git show HEAD:src/core/meio-b-gates.ts`
...
### `evaluateMeioBRendaCriteria` — 4 ramos sequenciais

```
1. regime_trabalho ou renda_principal ausente
   → can_advance=false, next_objective=coletar_{primeiro ausente}
   → G_FATO_CRITICO_AUSENTE

2. autonomo_ir_required (regime=autonomo) E autonomo_tem_ir NÃO detectado
   → can_advance=false, next_objective=coletar_autonomo_tem_ir
   → G_REGIME_RENDA

3. low_income_solo_signal (processo=solo E renda < 3000)
   → can_advance=false, next_objective=avaliar_composicao_renda
   → G_REGIME_RENDA

4. Todos ok
   → can_advance=true, authorized_next_step=qualification_eligibility
```

### `evaluateMeioBElegibilidadeCriteria` — 3 ramos sequenciais

```
1. nacionalidade ausente
   → can_advance=false, next_objective=coletar_nacionalidade
   → G_FATO_CRITICO_AUSENTE

2. nacionalidade=estrangeiro E rnm_status != 'valido'
   → can_advance=false, next_objective=validar_rnm
   → G_ELEGIBILIDADE
...
`low_income_solo_signal` depende de `processo` (do Meio A) sendo persistido nos facts.
Se `processo` não chegou ao Meio B via `state.facts`, o sinal não dispara mesmo com renda baixa.
