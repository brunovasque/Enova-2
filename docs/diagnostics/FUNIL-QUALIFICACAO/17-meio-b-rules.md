# 17 — Conteúdo completo: src/core/meio-b-rules.ts

**Data:** 2026-05-06
**Fonte:** `git show HEAD:src/core/meio-b-rules.ts`
...
| Constante | Facts |
|---|---|
| `MEIO_B_RENDA_REQUIRED_FACTS` | `regime_trabalho`, `renda_principal` |
| `MEIO_B_ELIGIBILITY_REQUIRED_FACTS` | `nacionalidade` |
| `MEIO_B_OPTIONAL_FACTS` | `autonomo_tem_ir`, `ctps_36`, `rnm_status` |

### Tipos

| Tipo | Valores |
|---|---|
| `RegimeTrabalho` | `clt`, `autonomo`, `aposentado`, `servidor`, `informal`, `multiplo` |
| `Nacionalidade` | `brasileiro`, `estrangeiro`, `naturalizado` |
| `RnmStatus` | `valido`, `vencido`, `ausente`, `indeterminado` |

### Condições de bloqueio

| Código | Condição |
|---|---|
| `meio_b.facto_critico_ausente` | `regime_trabalho` ou `renda_principal` ausente |
| `meio_b.autonomo_sem_ir` | `regime_trabalho=autonomo` sem `autonomo_tem_ir` confirmado |
| `meio_b.renda_solo_baixa` | `processo=solo` com `renda_principal < 3000` |
| `meio_b.rnm_invalido_ou_ausente` | `nacionalidade=estrangeiro` sem `rnm_status=valido` |
