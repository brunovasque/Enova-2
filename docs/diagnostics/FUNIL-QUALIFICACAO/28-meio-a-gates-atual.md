# 28 — meio-a-gates.ts — conteúdo atual

**Data:** 2026-05-08
**Arquivo:** `src/core/meio-a-gates.ts`
...
| Gate | Condição | next_objective | activated_gates |
|---|---|---|---|
| G1 — Fato crítico ausente | `!estado_civil_detected` ou `!processo_detected` | `coletar_{estado_civil\|processo}` | `G_FATO_CRITICO_AUSENTE` |
| G2 — Processo inválido para casado civil | `estado_civil='casado_civil'` e `processo != 'conjunto'` | `corrigir_processo_para_conjunto` | `G_COMPOSICAO_FAMILIAR` |
| G3 — Composição sem ator | `composition_required` e `!composition_actor_detected` | `coletar_composition_actor` | `G_COMPOSICAO_FAMILIAR` |
| G3B — Estado civil P3 ausente | `composition_actor_detected` e `actor != conjuge` e `!estado_civil_p3_detected` | `coletar_estado_civil_p3` | `G_COMPOSICAO_FAMILIAR` |
| G4 — Dependentes sem quantidade | `dependents_required` e `!dependents_count_detected` | `coletar_dependents_count` | `G_COMPOSICAO_FAMILIAR` |
| ADVANCE | Todos os gates passam | `avancar_para_qualification_renda` | `[]` |

## Importações

- `MeioASignals` — de `meio-a-parser.ts`
- `MEIO_A_ADVANCE_CRITERIA`, `MEIO_A_BLOCKING_CONDITIONS`, `MEIO_A_NEXT_STEP`, `MEIO_A_SIGNAL_POLICY` — de `meio-a-rules.ts`
