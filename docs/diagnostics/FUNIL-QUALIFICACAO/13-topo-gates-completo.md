# 13 — src/core/topo-gates.ts — Conteúdo Completo
...
### Interface TopoCriteriaResult — campos

| Campo | Tipo | Descrição |
|---|---|---|
| `can_advance` | boolean | true = topo completo, pode ir para qualification_civil |
| `authorized_next_step` | string | 'qualification_civil' ou 'discovery' |
| `criteria_code` | string | Código TOPO_BLOCKING_CONDITIONS ou TOPO_ADVANCE_CRITERIA |
| `structural_reason` | string | Log interno — nunca enviado ao cliente |
| `track_signal` | string | 'lead_no_trilho_principal' ou 'sinal_de_desvio_detectado' |
| `missing_required_facts` | string[] | Facts ausentes: 'customer_goal', 'nome_completo', 'nacionalidade', 'rnm_valido' |
| `next_objective` | string | Código opaco → SEMANTIC_MAP em semantic-next-objective.ts |

### evaluateTopoCriteria — 4 gates sequenciais

```
Gate 1: customer_goal_detected?    → NÃO → bloqueia (COLETAR_CUSTOMER_GOAL)
Gate 2: nome_completo_detected?    → NÃO → bloqueia (EXPLICAR_MCMV_E_COLETAR_NOME)
Gate 3: nacionalidade_detected?    → NÃO → bloqueia (PERGUNTAR_NACIONALIDADE)
Gate 4: estrangeiro + !rnm_valido? → SIM → bloqueia (PERGUNTAR_RNM)
         ↓ todos passaram
can_advance=true → authorized_next_step='qualification_civil'
```

### isTopoFactoCriticoAusente

Versão simplificada dos 4 gates — retorna boolean para uso no engine sem carregar o `TopoCriteriaResult`.

### Lacunas identificadas

- **LF-01**: Gate 4 (`rnm_valido`) é booleano — não distingue "não tem RNM" de "tem RNM com prazo determinado" (ver topo-parser.ts `resolveRnmValido`).
- **LF-03**: Não há gate para `inelegivel_rnm` — se cliente tem RNM inválido, o Core apenas bloqueia em `ESTRANGEIRO_SEM_RNM_VALIDO` indefinidamente sem encaminhar para encerramento.
